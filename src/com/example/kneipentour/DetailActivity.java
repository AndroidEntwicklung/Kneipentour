...
public class DetailActivity extends Activity {
	public void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			selected_name = (String) savedInstanceState
					.getSerializable("selected_name");
		}

		// get data
		Datenbank localDB = new Datenbank();
		ArrayList<KneipenListItem> kneipe_details = localDB.getListData(
				selected_name, "", "");
		...
		address_tv.setText(found_kneipe.getStreet() + ", "
				+ found_kneipe.getCity());
		rating_tv.setText(Html.fromHtml("<b>Rating:</b> " + found_kneipe.getRating()));
		review_tv.setText(Html.fromHtml("<b>Review:</b> " + found_kneipe.getReview()));
		...
		image_view.setImageUrl(found_kneipe.getImagepath(), mImageLoader);
		
		float ratio = image_view.getDrawable().getIntrinsicWidth() / image_view.getDrawable().getIntrinsicHeight();
		image_view.getLayoutParams().height = Math.round(image_view.getLayoutParams().width * ratio);
	}
}